...
def get_peft_model_state_dict(model, state_dict=None, adapter_name="default"):
    ...
    if config.peft_type in (PeftType.LORA, PeftType.ADALORA):
        ...
        if not config.scpp:
            ...
            # if config.inference_mode:
            to_return["prompt_weight"] = model.prompt_encoder[adapter_name].prompt_weight
...
def set_peft_model_state_dict(model, peft_model_state_dict, adapter_name="default"):
    ...
    """
    print("############ set_peft_model_state_dict ##############")
    config = model.peft_config[adapter_name]
    state_dict = {}
    # print("model.modules_to_save", model.modules_to_save) # None
    if model.modules_to_save is not None:
    ...
    if isinstance(config, PromptLearningConfig):
        if not config.scpp:
            ...
            print("########## load prompt weight ############")
            model.prompt_encoder[adapter_name].prompt_weight.data = peft_model_state_dict["prompt_weight"]
        ...
        if config.peft_type == PeftType.PROMPT_TUNING_LORA and config.load_lora_embeddings:
            if not config.scap:
                ...
                model.prompt_encoder[adapter_name].lora_weight.data = peft_model_state_dict["prompt_encoder.lora_weight"]
                if config.load_lora_embedding_B:
                    model.prompt_encoder[adapter_name].codebook_lora.data = peft_model_state_dict["prompt_encoder.codebook_lora"]
    return load_result
