...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def get_prompt_embedding_to_save(self, adapter_name):
        ...
        if peft_config.peft_type == PeftType.PROMPT_TUNING_LORA:
            if not prompt_encoder.pq_lora:
                ...
                prompt_tokens = (
                    self.prompt_tokens[adapter_name].unsqueeze(0).expand(1, -1).to(prompt_encoder.lora_weight.device)
                )
        else:
            prompt_tokens = (
        ...
        if peft_config.peft_type == PeftType.PROMPT_TUNING_LORA:
            if not prompt_encoder.pq_lora:
                prompt_tokens = (
                    ...
                    .unsqueeze(0)
                    .expand(batch_size, -1)
                    .to(prompt_encoder.lora_embedding_A.device)
                )
            else:
                prompt_tokens = (
                    self.prompt_tokens[self.active_adapter]
                    .unsqueeze(0)
                    .expand(batch_size, -1)
                    .to(prompt_encoder.lora_weight.device)
                )
        else: 
            prompt_tokens = (
                self.prompt_tokens[self.active_adapter]
            ...
            if peft_config.inference_mode:
                if peft_config.peft_type == PeftType.PROMPT_TUNING_LORA:
                    if  prompt_encoder.pq_prompt:
                        ...
                        # (40, 48, 16)
                        prompt_quantized = torch.sum(prompt_weight * torch.unsqueeze(prompt_encoder.codebook, 0), dim=2)
                        # (40, 768)
    ...
    def load_adapter(self, model_id, adapter_name, is_trainable=False, **kwargs):
        ...
        if os.path.exists(os.path.join(path, SAFETENSORS_WEIGHTS_NAME)): # adapter_model.bin
        ...
        if peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            if inputs_embeds is None:
                ...
                if self.prompt_encoder[self.active_adapter].pq_lora:
                    ...
                    # (256, 768)
                    lora_quantized = lora_quantized.view(self.prompt_encoder[self.active_adapter].max_length, -1)
                    # scaling = self.prompt_encoder[self.active_adapter].scaling
                    inputs_embeds += lora_quantized.unsqueeze(0).repeat(batch_size, 1, 1)
                else:
                    ...
                    scaling = self.prompt_encoder[self.active_adapter].scaling
                    # add lora matrix weights into the original word embeddings
    ...
    def forward(
        ...
        **kwargs,
        ...
        if peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            # Token_type_ids are 0s for the first sentence and 1 for the second sentence.
            if kwargs.get("token_type_ids", None) is not None:
                kwargs["token_type_ids"] = torch.cat(
                    (
                        torch.zeros(batch_size, peft_config.num_virtual_tokens).to(self.word_embeddings.weight.device),
