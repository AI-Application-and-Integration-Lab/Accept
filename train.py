...
class DataTrainingArguments:
    ...
    split_validation_test: Optional[bool] = field(default=True,
                                                  metadata={"help": "If set, for the datasets which do not"
                                                                    ...
                                                                    " datasets)."})
    k_shot_example: Optional[int] = field( # adjust number of shot
        default=None,
        metadata={
...
class ModelArguments:
    ...
    pretrain_init: bool = field(
        default=False,
        metadata={"help": "Whether to use pretrain initialization for pq-both tuning or not."}
    )
...
def main():
    ...
    # log_level = logging.ERROR # disable warning
    ...
    def compute_metrics_decoder(p: EvalPrediction, processor: PostProcessor, metrics: Callable):
        ...
        preds = output_sequences[:, data_args.max_seq_length:] # select the text behind input text
        decoded_preds, decoded_labels = processor.process(preds, labels, data_info=None)
        result = {}
        for metric in metrics:
            result.update(metric(decoded_preds, decoded_labels))
    ...
    if any(x in model_args.model_name_or_path for x in ["bert", "roberta", "albert"]):
    ...
    elif any(x in model_args.model_name_or_path for x in ["gpt", "llama", "Qwen"]):
        ...
        # training_args.generation_max_length: 20 (default)
        # data_args.max_seq_length: 64 (define in config file)
    ...
    if model_args.peft_model_id:
        ...
        elif task_type == TaskType.CAUSAL_LM and ("llama" in model_args.model_name_or_path or "Qwen" in model_args.model_name_or_path):
        ...
        # 把 load 好 weight 的 base model 以及 model path 傳進 peftmodel
        model = PeftModel.from_pretrained(model, model_args.peft_model_id)

        # 剩下 adapter 的地方有 gradient
        ...
        if task_type == TaskType.SEQ_CLS:
        ...
        elif task_type == TaskType.CAUSAL_LM and "Qwen" in model_args.model_name_or_path:
            model = AutoModelForCausalLM.from_pretrained(model_args.model_name_or_path,
            ...
            model = LlamaForCausalLM.from_pretrained(
                ...
                return_dict=True,
                # load_in_8bit=training_args.quantization,
                torch_dtype=torch.float16, # 國網不能跑 bf16
                device_map="auto",
                low_cpu_mem_usage=True,
            )
        model_args.hidden_size = model.config.hidden_size
        if model_args.peft_type == "PROMPT_TUNING":
            ...
            peft_config = PromptTuningLoRAConfig(
                task_type=task_type,
                prompt_tuning_init=PromptTuningInit.TEXT if model_args.text_init else PromptTuningInit.RANDOM,
                pretrain_init=model_args.pretrain_init,
                num_virtual_tokens=model_args.prefix_length,
                num_transformer_submodules=model_args.num_transformer_submodules,
                prompt_tuning_init_text=model_args.prompt_tuning_init_text,
    ...
    if model_args.peft_type == "PROMPT_TUNING_LORA" and training_args.added_embedding_lr is not None:
        ...
        for name, param in model.named_parameters():
            if name in ["prompt_encoder.default.lora_embedding_A", "prompt_encoder.default.lora_embedding_B",
                        "prompt_encoder.default.lora_weight", "prompt_encoder.default.codebook_lora"]:
